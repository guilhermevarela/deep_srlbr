#!/usr/bin/perl -w
# baseline: baseline result generator for CoNLL-2004 shared task
# usage: paste -d' ' words.dev synt.dev.pred props.dev | baseline
            ...
            if ($buffer[$i] =~ /^\w[\w\-]*\s/) {
               ...
               while ($j >= 0 and $pos[$j] =~ /[A-Z]/ and $pos[$j] ne "CC" and
                  ...
                  if ($words[$j] =~ /^(that|which|who)$/) { 
                     $buffer[$j] =~ s/\s(\S+)$/ (R-$tag$1R-$tag)/;
                  }
                  $j--; 
               }
               if ($j >= 0 and $chunk[$j] =~ /-NP$/) {
                  $buffer[$j] =~ s/\s(\S+)$/ $1$tag)/;
