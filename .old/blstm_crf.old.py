...
def forward(X, sequence_length, hidden_size):
    '''
        ...
        args:
            ...
            sequence_length:[batch_size] tensor (int)
                carrying the size of each sequence
...
if __name__ == '__main__':
    ...
    def check_embeddings(value):
        if not(re.search(r'^[a-z0-9]*\_s\d+$', value)):
            raise argparse.ArgumentTypeError(
                "{:} is an invalid embeddings".format(value)
            )
        else:
    ...
    parser = argparse.ArgumentParser(
        description='''Script used for customizing
        inputs for the bi-LSTM model and using CRF.''')

    parser.add_argument(
        'depth', type=int, nargs='+', default=HIDDEN_SIZE,
        help='''Set of integers corresponding the layer
        sizes on MultiRNNCell\n''')

    parser.add_argument(
        '--embeddings', dest='embeddings_model',
        type=check_embeddings, nargs=1, default=EMBEDDING_MODEL,
        help='''embedding model name and size in format
        <embedding_name>_s<embedding_size>. Examples:
        glove_s50, wang2vec_s100\n''')
    ...
    input_sequence_features= ['ID', 'LEMMA', 'MARKER', 'PRED_1', 'GPOS']  
    if ctx_p > 0:
