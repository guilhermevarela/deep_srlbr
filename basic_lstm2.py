...
class BasicLSTM(object):
	def __init__(self, hidden_sz=512, embedding='glove_s50'):
		...
		self.index_propbgn, self.index_propfnl= self._minibatch()
	...
	def train(self, lr=1e-3, epochs=100,  display_step=10):
		...
		Xtrain= self.df_train.as_matrix()
		Ytrain= self.Ytrain
		with tf.Session() as session:
			...
			for e in range(epochs):
				...
				perm= np.random.permutation(self.n_examples_train)
				idx_bgn = [self.index_propbgn[i] for i in perm]
				idx_fnl = [self.index_propfnl[i] for i in perm]
				for j in range(self.n_examples_train):
					Xbatch=self._embedding_lookup(Xtrain[idx_bgn[j]:idx_fnl[j],:])
					Ybatch=Ytrain[idx_bgn[j]:idx_fnl[j],:]
					nb= Xbatch.shape[0]
	...
	def _minibatch(self):				
		index_propbgn = list(self.df_train.drop_duplicates(subset=['P_S'], keep='first').index )
		index_propfnl = list(self.df_train.drop_duplicates(subset=['P_S'], keep='last').index )
		return index_propbgn, index_propfnl
